...
void FrameLess::mousePress(QMouseEvent* e) {
	if (e->button() & Qt::LeftButton) {
		...
		if (!testFlag(Edge::None, _mousePress)) {
			_rubberband->setGeometry(_target->frameGeometry());
		}
		if (_target->rect().marginsRemoved(QMargins(borderWidth() + 1, borderWidth() + 1, borderWidth() + 1, borderWidth() + 1)).contains(e->pos())) {
			_dragStart = true;
			_dragPos = e->globalPos();
		}
	}
}
...
void FrameLess::mouseMove(QMouseEvent* e)
{
	if (_dragStart)
	{
		_target->move(_target->pos() + e->globalPos() - _dragPos);
		_dragPos = e->globalPos();
	}

	if (testFlag(Edge::None, _mousePress))return;
...
void FrameLess::updateCursorShape(const QPoint& pos) {
	...
	if (!_leftButtonPressed) {
		...
		_cursorchanged = true;
		if (testFlag(Edge::Top, _mouseMove) || testFlag(Edge::Bottom, _mouseMove)) {
			_target->setCursor(Qt::SizeVerCursor);
		}
		else if (testFlag(Edge::Left, _mouseMove) || testFlag(Edge::Right, _mouseMove)) {
			_target->setCursor(Qt::SizeHorCursor);
		}
		else if (testFlag(Edge::TopLeft, _mouseMove) || testFlag(Edge::BottomRight, _mouseMove)) {
			_target->setCursor(Qt::SizeFDiagCursor);
		}
		else if (testFlag(Edge::TopRight, _mouseMove) || testFlag(Edge::BottomLeft, _mouseMove)) {
